...
## 5. aws에서 로드벨런싱 하는 방법 4가지 설명(피티에 있음)

## 6. rds 서비스 어떻게 하이 어베일러빌리티를 제공하는지 돈을 지불하면 이중화 / 프라이머리, 스텐바이 구조 이해 / 얼마나 안정적인지 /멀티 az

## 7. 오토스케일링에 대해 정의 설명(스케일 인, 스케일 아웃 함께 설명) aws에서 오토스케일링을 구현하려면 해야하는 구성요소 / 그림 / 설명 (피피티 참고) aws가 인스턴스 개수 조정하는 방법 4가지 설명(온디맨드 조절방식 cpu사용량 예 들기)
